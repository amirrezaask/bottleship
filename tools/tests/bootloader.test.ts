...
describe('bootloader layout', () => {
  ...
  test.each([0, 1, 11, 12, 22, 64])('preserves tables and calls for %i DLLs', (count) => {
    const dlls = Array.from({ length: count }, (_, i) => ({
      name: `dll${i}.dll`,
      baseAddress: 0x1000000 + i * 0x10000,
    ...
    expect([...code.slice(offset, offset + 11)]).toEqual([
      0xb8, 0x04, 0x00, 0xad, 0xde, 0xba, 0x77, 0xb0, 0x00, 0x00, 0xef,
    ]);
    offset += 11;
  ...
  test('rejects a DLL list beyond reserved low memory', () => {
    ...
    expect(() => createBootloader(0x401000, 0x1100000, Array(65).fill(dll))).toThrow(
      'DLL initialization exceeds the boot region',
    );
  });
});
