//! Page-guarded string leaves over v86's own guest RAM, without host calls or staging.
use std::ptr::{addr_of, copy_nonoverlapping, read_unaligned};
use crate::cpu::{bulk_memory::resident_span, memory};
#[cfg(target_feature = "simd128")]
use std::arch::wasm32::*;

// Match the existing handlers' scan cap, including the character at offset 0x100000.
const LIMIT: u32 = 0x100001;
static mut ENABLED: bool = true;
// strlen, wcslen, strcmp, stricmp, wcsicmp, strchr, strrchr, wcschr, strcpy, wcscpy, misses.
static mut STATS: [u32; 11] = [0; 11];
#[no_mangle]
pub extern "C" fn get_string_memory_abi() -> u32 { 1 }
#[no_mangle]
pub unsafe extern "C" fn set_string_memory_enabled(value: u32) { ENABLED = value != 0; }
#[no_mangle]
...
unsafe fn record<T>(value: Option<T>, hit: usize) -> Option<T> {
    let index = if value.is_some() { hit } else { 10 };
...
/// An odd-addressed wchar can straddle pages; then validate exactly its two bytes.
#[inline]
...
fn fold_vector<const WIDE: bool>(v: v128) -> v128 {
    // Preserve the parent's ASCII-only folding, including non-ASCII bytes/code units.
    let upper = if WIDE {
...
/// Read only validated page chunks. A NUL masks out all later matches in its vector.
unsafe fn scan<const WIDE: bool, const REVERSE: bool>(src: u32, target: u32) -> Option<u32> {
    ...
    while offset < LIMIT {
        ...
        let mut i = 0u32;
        // Empty strings and first-character hits avoid constructing SIMD masks.
        let first = unit::<WIDE>(p);
        if first == target { last = start; if !REVERSE { return Some(last); } }
        if first == 0 { return Some(last); }
        i += 1;
        #[cfg(target_feature = "simd128")]
        while (count - i) * stride >= 16 {
            ...
            if REVERSE {
                if zeros != 0 { matches &= (1u32 << (zeros.trailing_zeros() + 1)) - 1; }
                if matches != 0 { last = start + (i + 31 - matches.leading_zeros()) * stride; }
                if zeros != 0 { return Some(last); }
            } else {
                let stop = matches | zeros;
                if stop != 0 {
                    let lane = stop.trailing_zeros();
                    return Some(if matches & (1 << lane) != 0 { start + (i + lane) * stride } else { 0 });
                }
            }
            i += 16 / stride;
        }
        while i < count {
            ...
            if c == target { last = start + i * stride; if !REVERSE { return Some(last); } }
            if c == 0 { return Some(last); }
            i += 1;
        }
        offset += count;
    }
    None // The existing handler decides its historical cap/fault behavior.
}

pub unsafe fn try_length<const WIDE: bool>(src: u32) -> Option<u32> {
    record(scan::<WIDE, false>(src, 0).map(|end| (end - src) / step::<WIDE>()), if WIDE { 1 } else { 0 })
}
pub unsafe fn try_find<const WIDE: bool, const REVERSE: bool>(src: u32, target: u32) -> Option<u32> {
    record(scan::<WIDE, REVERSE>(src, target), if WIDE { 7 } else if REVERSE { 6 } else { 5 })
}
...
unsafe fn compare<const WIDE: bool, const FOLD: bool>(a: u32, b: u32) -> Option<i32> {
    ...
    while offset < LIMIT {
        ...
        let diff = if FOLD { fold(first) as i32 - fold(second) as i32 } else { first as i32 - second as i32 };
        if diff != 0 || first == 0 { return Some(diff); }
        i += 1;
        #[cfg(target_feature = "simd128")]
        while (count - i) * stride >= 16 {
            ...
            let eq = if FOLD { equal_mask::<WIDE>(fold_vector::<WIDE>(x), fold_vector::<WIDE>(y)) }
                else { equal_mask::<WIDE>(x, y) };
            ...
            if stop != 0 {
                ...
                return Some(if FOLD { fold(x) as i32 - fold(y) as i32 } else { x as i32 - y as i32 });
            }
            i += 16 / stride;
        }
        while i < count {
            ...
            let diff = if FOLD { fold(x) as i32 - fold(y) as i32 } else { x as i32 - y as i32 };
            if diff != 0 || x == 0 { return Some(diff); }
            i += 1;
        }
        offset += count;
    }
...
unsafe fn copy_string<const WIDE: bool>(dst: u32, src: u32) -> Option<()> {
    ...
    // Retain the original sequential behavior for undefined overlapping strcpy inputs.
    if dst < src_end && src < dst_end { return None; }
