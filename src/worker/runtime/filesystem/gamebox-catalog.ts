...
function trustStoreKey(store: GameboxTrustStore | undefined, keyId: string): string | undefined {
  ...
  if (typeof store !== 'object' || Array.isArray(store)) throw new Error('Invalid GameBox trust store');
...
export async function verifyGameboxSignature(
  ...
  if (!HASH.test(bundleHash) || !Number.isSafeInteger(sourceFormatVersion) || sourceFormatVersion < 1 || sourceFormatVersion > 0xffffffff) {
  ...
  if (!SIGNATURE.test(signature)) return { status: 'untrusted', reason: 'invalid-signature', keyId };
  let publicKeyHex: string | undefined;
  try {
    publicKeyHex = trustStoreKey(trustStore, keyId);
  } catch (error) {
    return { status: 'untrusted', reason: error instanceof Error ? error.message : String(error), keyId };
  }
...
function exportedChecksumRecords(bytes: Uint8Array): ExportedChecksumRecord[] {
  ...
  for (let index = 0; index < count; index++) {
    ...
    offset += 8;
    if (sizeBig > BigInt(Number.MAX_SAFE_INTEGER)) throw new Error('Exported checksum size is unsafe');
...
async function verifyExportedSourceRoot(
  ...
  const checksumBytes = await readGameboxJsonBytes(archive, 'gamebox/checksums.bin', MAX_CHECKSUM_BYTES);
  if ((await sha256Hex(checksumBytes)) !== bundleHash) throw new Error('exported checksum root mismatch');
  ...
  for (const record of records) {
    ...
    if (!entry || entry.isDirectory || entry.compression !== 0 || entry.uncompressedSize !== record.size)
      throw new Error(`missing exported source artifact: ${record.path}`);
    const bytes = await archive.readEntry(entry);
    if ((await sha256Hex(bytes)) !== record.sha256) throw new Error(`exported source artifact mismatch: ${record.path}`);
  }
  ...
  for (const value of rawCatalog.files) {
    ...
    if (!expected || expected.path !== path || expected.hash !== objectHash || expected.size !== objectSize)
  ...
  for (const [objectHash, objectSize] of objects) {
    if (catalogObjects.get(objectHash) !== objectSize) throw new Error('exported catalog object binding mismatch');
  }
  ...
  if (actualStatic.size !== expectedStatic.size) throw new Error('exported static artifact set mismatch');
  ...
  static async open(
    ...
    if (preparedTrust.status === 'trusted') {
      ...
      try {
        ...
        preparedTrust = {
          status: 'untrusted',
          reason: error instanceof Error ? `exported-root-${error.message}` : 'exported-root-invalid',
          keyId,
        };
      }
    }
    ...
    const staticArtifacts = ((catalog.staticArtifacts as unknown[] | undefined) ?? []).map(
      (value) => {
        ...
        if (!moduleHashes.has(moduleHash) || bytes < 8 || bytes > 32 * 1024 * 1024)
    ...
    if (
      catalog.filesystemProfile !== undefined &&
      catalog.filesystemProfile !== GAMEBOX_FILESYSTEM_PROFILE_PATH
    )
