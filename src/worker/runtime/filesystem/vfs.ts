...
import { gameBoxFilesystemProfile } from '../../core/diagnostics/gamebox-filesystem-profile';
...
export class VirtualFileSystem {
  ...
  mountRom(archive: ZipArchive, romPrefix: string, index: Map<string, ZipEntry>): void {
    ...
    Logger.verbose(
      LogCategory.SYSTEM,
      `VFS: ROM mounted, prefix="${this.romPrefix}", index size=${this.romIndex.size}`,
    );
    ...
    if (driveRelative) {
      ...
      if (tail.startsWith('\\')) {
        ...
        if (!tail) {
          ...
          const base = currentDirOnDrive.endsWith('\\')
            ? currentDirOnDrive
            : `${currentDirOnDrive}\\`;
          absolute = `${base}${tail}`;
        }
      ...
      absolute = trimmed.startsWith('\\') ? `C:${trimmed}` : `${this.currentDir}${trimmed}`;
    }
  ...
  setCdRedirect(guestPath: string | null): void {
    this.cdRedirect =
      guestPath && guestPath.trim() ? guestPath.trim().replace(/[\\/]+$/, '') : null;
  }
  ...
  private openSyncUnprofiled(
    ...
    if (createDisposition === 3) {
      if (!this.parentDirectoryExists(full)) {
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: openSync("${path}") OPEN_EXISTING parent missing for "${full}"`,
        );
        return null;
      }
      if (existsInRom && hasOverlay) {
        ...
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: openSync("${path}") -> overlay (overlay takes priority over ROM)`,
        );
      ...
      if (existsInRom) {
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: openSync("${path}") -> found in ROM as "${relRom}"`,
        );
        return { kind: 'file' as const, path: full, position: 0, access, source: 'rom' };
      }
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: openSync("${path}") -> NOT found (relRom="${relRom}", existsInRom=${existsInRom})`,
      );
      return null;
    }
    ...
    if (!this.overlay) {
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: openSync("${path}") create disposition=${createDisposition} failed: overlay not initialized`,
      );
      return null;
    }
    ...
    if (!this.parentDirectoryExists(full)) {
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: openSync("${path}") create disposition=${createDisposition} parent missing for "${full}"`,
      );
      return null;
    }
    ...
    if (createDisposition === 1) {
      if (hasOverlay || existsInRom) {
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: openSync("${path}") CREATE_NEW failed: file already exists`,
        );
        return null;
      }
    ...
    if (createDisposition === 4) {
      ...
      const resolvedPath =
        source === 'overlay' ? (this.overlay.resolveExistingPath(full) ?? full) : full;
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: openSync("${path}") -> OPEN_ALWAYS source=${source}`,
      );
    ...
    if (createDisposition === 5) {
      if (!hasOverlay && !existsInRom) {
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: openSync("${path}") TRUNCATE_EXISTING failed: file not found`,
        );
        return null;
      }
      this.clearRomWhiteout(full);
  ...
  openSync(path: string, access: number, disposition: number): VfsFileHandle | null {
    const result = this.openSyncUnprofiled(path, access, disposition);
    if (gameBoxFilesystemProfile.isRunning())
      gameBoxFilesystemProfile.recordOpen(
        result?.path ?? path,
  ...
  private async openUnprofiled(
    path: string,
    access: number,
    disposition: number,
  ): Promise<VfsFileHandle | null> {
    Logger.verbose(
      LogCategory.SYSTEM,
      `VFS: open("${path}", access=0x${access.toString(16)}, disposition=0x${disposition.toString(16)}) starting`,
    );
    try {
      // Try sync fast-path first
      const syncResult = this.openSyncUnprofiled(path, access, disposition);
      if (syncResult !== null) {
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") completed via sync fast-path, source=${syncResult.source}`,
        );
        return syncResult;
      }
      ...
      if (createDisposition !== 3 && !this.parentDirectoryExists(full)) {
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") create disposition=${createDisposition} parent missing`,
        );
        return null;
      }
      ...
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: open("${path}") -> full="${full}", relRom="${relRom}", existsInOverlay=${existsInOverlay}, existsInRom=${existsInRom}, disposition=${createDisposition}`,
      );
      ...
      if (createDisposition === 1) {
        ...
        if (existsInOverlay || existsInRom) {
          Logger.verbose(
            LogCategory.SYSTEM,
            `VFS: open("${path}") CREATE_NEW failed: file already exists`,
          );
          return null;
        }
        Logger.warn(
          LogCategory.SYSTEM,
          `VFS: open("${path}") CREATE_NEW not fully handled in async path`,
        );
      }
      if (createDisposition === 2) {
        // CREATE_ALWAYS
        if (!overlay) {
          Logger.error(
            LogCategory.SYSTEM,
            `VFS: open("${path}") CREATE_ALWAYS failed: overlay not initialized`,
          );
          return null;
        }
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") CREATE_ALWAYS: calling truncateFile`,
        );
        this.clearRomWhiteout(full);
        ...
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") CREATE_ALWAYS: truncateFile completed`,
        );
      ...
      if (createDisposition === 3) {
        ...
        if (!existsInOverlay && !existsInRom) {
          Logger.verbose(
            LogCategory.SYSTEM,
            `VFS: open("${path}") OPEN_EXISTING failed: file not found`,
          );

          // Diagnostic: Search for this filename in any ROM directory
          const fileName = path.split(/[\\/]/).pop()?.toLowerCase();
          if (fileName) {
            const matches = Array.from(this.romIndex.keys()).filter(
              (k) => k.toLowerCase().endsWith('/' + fileName) || k.toLowerCase() === fileName,
            );
            if (matches.length > 0) {
              Logger.verbose(
                LogCategory.SYSTEM,
                `VFS: Diagnostic - Found potential matches in ROM: ${matches.join(', ')}`,
              );
            }
          }
        ...
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") OPEN_EXISTING: returning handle, source=${source}`,
        );
        const resolvedPath =
          source === 'overlay' ? (overlay?.resolveExistingPath(full) ?? full) : full;
      ...
      if (createDisposition === 4) {
        ...
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") OPEN_ALWAYS: checking if file exists`,
        );
        if (!overlay) {
          Logger.error(
            LogCategory.SYSTEM,
            `VFS: open("${path}") OPEN_ALWAYS failed: overlay not initialized`,
          );
          return null;
        }
        if (!existsInOverlay && !existsInRom) {
          Logger.verbose(
            LogCategory.SYSTEM,
            `VFS: open("${path}") OPEN_ALWAYS: file doesn't exist, creating via truncateFile`,
          );
          this.clearRomWhiteout(full);
          await overlay.truncateFile(full);
          Logger.verbose(
            LogCategory.SYSTEM,
            `VFS: open("${path}") OPEN_ALWAYS: truncateFile completed`,
          );
        } else {
          Logger.verbose(
            LogCategory.SYSTEM,
            `VFS: open("${path}") OPEN_ALWAYS: file exists, opening without truncate`,
          );
        }
        const source = existsInOverlay ? 'overlay' : existsInRom ? 'rom' : 'overlay';
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") OPEN_ALWAYS: returning handle, source=${source}`,
        );
        const resolvedPath =
          source === 'overlay' ? (overlay.resolveExistingPath(full) ?? full) : full;
        return { kind: 'file', path: resolvedPath, position: 0, access, source };
      ...
      if (createDisposition === 5) {
        ...
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") TRUNCATE_EXISTING: checking if file exists`,
        );
        if (!overlay) {
          Logger.error(
            LogCategory.SYSTEM,
            `VFS: open("${path}") TRUNCATE_EXISTING failed: overlay not initialized`,
          );
          return null;
        }
        if (!existsInOverlay && !existsInRom) {
          Logger.verbose(
            LogCategory.SYSTEM,
            `VFS: open("${path}") TRUNCATE_EXISTING failed: file doesn't exist`,
          );
          return null;
        }
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") TRUNCATE_EXISTING: calling truncateFile`,
        );
        this.clearRomWhiteout(full);
        await overlay.truncateFile(full);
        if (existsInRom) overlay.markShadowed(full);
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") TRUNCATE_EXISTING: truncateFile completed`,
        );
        return { kind: 'file', path: full, position: 0, access, source: 'overlay' };
      }
      ...
      Logger.warn(
        LogCategory.SYSTEM,
        `VFS: open("${path}") unhandled disposition=${createDisposition}, using default logic`,
      );
      const source = existsInOverlay ? 'overlay' : existsInRom ? 'rom' : 'overlay';
      if (source === 'overlay' && overlay && !existsInOverlay) {
        Logger.verbose(
          LogCategory.SYSTEM,
          `VFS: open("${path}") default case: creating file via truncateFile`,
        );
        this.clearRomWhiteout(full);
        await overlay.truncateFile(full);
      }
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: open("${path}") default case: returning handle, source=${source}`,
      );
      const resolvedPath =
        source === 'overlay' ? (overlay?.resolveExistingPath(full) ?? full) : full;
      return { kind: 'file', path: resolvedPath, position: 0, access, source };
      ...
      Logger.error(
        LogCategory.SYSTEM,
        `VFS: open("${path}", access=0x${access.toString(16)}, disposition=0x${disposition.toString(16)}) failed: ${e}`,
      );
      throw e;
    }
  }
  ...
  async open(path: string, access: number, disposition: number): Promise<VfsFileHandle | null> {
    try {
      const result = await this.openUnprofiled(path, access, disposition);
      if (gameBoxFilesystemProfile.isRunning())
        gameBoxFilesystemProfile.recordOpen(
          result?.path ?? path,
          result?.source ?? 'unknown',
      ...
      if (gameBoxFilesystemProfile.isRunning())
        gameBoxFilesystemProfile.recordOpen(path, 'unknown', false);
      throw error;
    }
  }
   ...
   */
  private readSyncUnprofiled(handle: VfsFileHandle, length: number): Uint8Array | null {
  ...
  private async readUnprofiled(handle: VfsFileHandle, length: number): Promise<Uint8Array> {
    if (length <= 0) {
    ...
    Logger.verbose(
      LogCategory.SYSTEM,
      `VFS: read(handle.path="${handle.path}", position=${handle.position}, length=${length}) starting`,
    );
    const startPos = handle.position;
    const sync = this.readSyncUnprofiled(handle, length);
    if (sync) {
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: read("${handle.path}") completed via sync fast-path, read ${sync.length} bytes`,
      );
      return sync;
    }
    ...
    try {
      ...
      if (
        handle.prefetchPromise &&
        handle.prefetchOffset !== undefined &&
        offset >= handle.prefetchOffset
      ) {
        ...
        if (buffered !== null) {
          ...
          Logger.verbose(
            LogCategory.SYSTEM,
            `VFS: read("${handle.path}") completed from prefetched window, read ${buffered.length} bytes, new position=${handle.position}`,
          );
          return buffered;
        }
      }
      ...
      handle.buffer =
        dataWindow.byteLength <= VirtualFileSystem.PREFETCH_CHUNK_SIZE ? dataWindow : undefined;
      ...
      Logger.error(
        LogCategory.SYSTEM,
        `VFS: read("${handle.path}", position=${handle.position}, length=${length}) failed: ${e}`,
      );
      throw e;
    }
  }
  ...
  readSync(handle: VfsFileHandle, length: number): Uint8Array | null {
    const startPos = handle.position;
    const result = this.readSyncUnprofiled(handle, length);
    if (gameBoxFilesystemProfile.isRunning() && result !== null)
      gameBoxFilesystemProfile.recordRead(
        handle.path,
        handle.source,
        startPos,
        length,
  ...
  async read(handle: VfsFileHandle, length: number): Promise<Uint8Array> {
    const startPos = handle.position;
    try {
      const result = await this.readUnprofiled(handle, length);
      if (gameBoxFilesystemProfile.isRunning())
        gameBoxFilesystemProfile.recordRead(
          handle.path,
          handle.source,
      ...
      if (gameBoxFilesystemProfile.isRunning())
        gameBoxFilesystemProfile.recordRead(handle.path, handle.source, startPos, length, 0, false);
      throw error;
    }
  }
   ...
   */
  private readIntoSyncUnprofiled(
   ...
   */
  private async readIntoUnprofiled(
    handle: VfsFileHandle,
    target: Uint8Array,
    targetOffset: number,
    length: number,
  ): Promise<number> {
    const sync = this.readIntoSyncUnprofiled(handle, target, targetOffset, length);
    if (sync !== null) {
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: readInto("${handle.path}") sync fast-path, read ${sync} bytes`,
      );
      return sync;
    }
    ...
    try {
      while (totalRead < length) {
        ...
        const data = await this.readUnprofiled(handle, chunk);
      ...
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: readInto("${handle.path}") async, read ${totalRead} bytes`,
      );
      return totalRead;
    } catch (e) {
      Logger.error(
        LogCategory.SYSTEM,
        `VFS: readInto("${handle.path}", position=${startPos}, length=${length}) failed: ${e}`,
      );
      throw e;
    }
  }
  ...
  readIntoSync(
    ...
    const startPos = handle.position;
    const result = this.readIntoSyncUnprofiled(handle, target, targetOffset, length);
    if (gameBoxFilesystemProfile.isRunning() && result !== null)
      gameBoxFilesystemProfile.recordRead(handle.path, handle.source, startPos, length, result);
    return result;
  }
  ...
  async readInto(
    ...
    const startPos = handle.position;
    try {
      const result = await this.readIntoUnprofiled(handle, target, targetOffset, length);
      if (gameBoxFilesystemProfile.isRunning())
        gameBoxFilesystemProfile.recordRead(handle.path, handle.source, startPos, length, result);
      return result;
    } catch (error) {
  ...
  writeSync(handle: VfsFileHandle, data: Uint8Array): number {
    ...
    try {
      ...
      if (harnessBus.fileEvents && written > 0)
        harnessBus.emit('fileWritten', {
          path: handle.path,
  ...
  async write(handle: VfsFileHandle, data: Uint8Array): Promise<number> {
    Logger.verbose(
      LogCategory.SYSTEM,
      `VFS: write(handle.path="${handle.path}", position=${handle.position}, size=${data.length}) starting`,
    );
    this.invalidateReadWindow(handle);
    if (!this.overlay) {
      Logger.warn(
        LogCategory.SYSTEM,
        `VFS: write("${handle.path}") failed: overlay not initialized`,
      );
      return 0;
    }
    try {
      ...
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: write("${handle.path}") completed: wrote ${written} bytes in ${writeTime.toFixed(2)}ms, new position=${handle.position}`,
      );
      return written;
    } catch (e) {
      Logger.error(
        LogCategory.SYSTEM,
        `VFS: write("${handle.path}", position=${handle.position}, size=${data.length}) failed: ${e}`,
      );
      throw e;
    }
  }
  ...
  async storeExecutable(data: Uint8Array, filename: string = 'app.exe'): Promise<void> {
    ...
    try {
      ...
      Logger.verbose(
        LogCategory.SYSTEM,
        `VFS: Stored executable "${filename}" (${data.length} bytes)`,
      );
    } catch (e) {
  ...
  private setPositionUnprofiled(handle: VfsFileHandle, offset: number, method: number): number {
  ...
  setPosition(handle: VfsFileHandle, offset: number, method: number): number {
    const oldPosition = handle.position;
    const result = this.setPositionUnprofiled(handle, offset, method);
    if (gameBoxFilesystemProfile.isRunning())
      gameBoxFilesystemProfile.recordSeek(handle.path, handle.source, oldPosition, result);
    return result;
  }

  private getFileSizeUnprofiled(path: string): number {
  ...
  getFileSize(path: string): number {
    const result = this.getFileSizeUnprofiled(path);
    if (gameBoxFilesystemProfile.isRunning())
  ...
  private readOverlayRomUnderlaySync(
    path: string,
    offset: number,
    length: number,
  ): Uint8Array | null {
  ...
  private async readOverlayRomUnderlayAsync(
    path: string,
    offset: number,
    length: number,
  ): Promise<Uint8Array> {
  ...
  private listDirectoryUnprofiled(path: string): VfsEntry[] {
    const full = this.resolvePath(path);
  ...
  listDirectory(path: string): VfsEntry[] {
    const result = this.listDirectoryUnprofiled(path);
    if (gameBoxFilesystemProfile.isRunning()) {
      const source = result[0]?.source ?? 'unknown';
      gameBoxFilesystemProfile.recordEnumeration(path, source, result.length, true);
    }
    return result;
  }
   ...
   */
  private statEntryUnprofiled(path: string): VfsEntry | null {
    const full = normalizePath(this.resolvePath(path));
  ...
  statEntry(path: string): VfsEntry | null {
    const result = this.statEntryUnprofiled(path);
    if (gameBoxFilesystemProfile.isRunning())
      gameBoxFilesystemProfile.recordStat(
        result?.path ?? path,
        result?.source ?? 'unknown',
        result !== null,
      );
  ...
  private async readRom(path: string, offset: number, length: number): Promise<Uint8Array> {
    ...
    if (!data) {
      ...
      if (pending) {
        ...
        const loadPromise = this.romArchive
          .readEntry(entry)
      ...
      if (generation !== this.romGeneration)
        throw new Error('ROM read cancelled during game switch');
  ...
  private async _prefetchEntry(rel: string, entry: ZipEntry): Promise<void> {
    ...
    const loadPromise = this.romArchive
      .readEntry(entry)
      .then((data) => {
        if (generation === this.romGeneration) this.addRomCache(key, data);
        return data;
      })
    ...
    const workers = Array.from({ length: Math.min(concurrency, queue.length || 1) }, async () => {
      while (queue.length > 0 && generation === this.romGeneration) {
        ...
        if (
          entry &&
          !entry.isDirectory &&
          entry.uncompressedSize <= this.MAX_CACHE_ENTRY_SIZE &&
          !this.romCache.has(key)
        ) {
          try {
            await this._prefetchEntry(rel, entry);
            prefetched++;
          } catch (_) {
            /* best-effort — range request issues are non-fatal */
          }
        }
  ...
  async pinRomFiles(rels: string[], concurrency = 8): Promise<number> {
    ...
    const workers = Array.from({ length: Math.min(concurrency, queue.length || 1) }, async () => {
      while (queue.length > 0) {
        if (this.romPinnedBytes >= this.PIN_TOTAL_MAX) {
          queue.length = 0;
          break;
        }
        const rel = queue.shift()!;
        ...
        try {
          ...
          /* best-effort — a missing/short read just stays unpinned */
        }
      }
    });
    await Promise.all(workers);
    if (pinned > 0) {
      Logger.log(
        LogCategory.SYSTEM,
        `VFS: pinned ${pinned} small ROM files for sync reads (${(this.romPinnedBytes / 1024) | 0} KB)`,
      );
    }
    return pinned;
  }
  ...
  startProgressivePrefetch(signal?: AbortSignal): void {
    this._runProgressivePrefetch(signal).catch(() => {
      /* best-effort */
    });
  }
  ...
  private async _runProgressivePrefetch(signal?: AbortSignal): Promise<void> {
    ...
    for (const [rel, entry] of entries) {
      if (
        signal?.aborted ||
        generation !== this.romGeneration ||
        fetchedBytes + entry.uncompressedSize > this.ROM_CACHE_MAX_BYTES
      )
        return;
  ...
  private maybeSchedulePrefetch(handle: VfsFileHandle): void {
    ...
    handle.prefetchPromise = this.fetchRange(
      handle,
      nextOffset,
      VirtualFileSystem.PREFETCH_CHUNK_SIZE,
  ...
  private async fetchRange(
    ...
    const raw =
      handle.source === 'overlay' && this.overlay
    ...
    return view.byteLength <= VirtualFileSystem.PREFETCH_CHUNK_SIZE &&
...
class OpfsOverlay {
  ...
  setPolicy(policy: PathPolicy): void {
    this.policy = policy;
  }
  ...
  private ephemeralWrite(key: string, path: string, offset: number, data: Uint8Array): number {
    ...
    this.entries.set(key, {
      size: Math.max(existing?.size ?? 0, end),
      kind: 'file',
  ...
  writeFileSync(path: string, offset: number, data: Uint8Array): number {
    ...
    if (
      isSequential &&
      cacheEntry.memoryBuffer.length + data.length < this.WRITE_BUFFER_THRESHOLD
    ) {
    ...
    for (const segment of segments) {
      try {
        ...
        if (e.name === 'TypeMismatchError') {
          Logger.warn(
            LogCategory.SYSTEM,
            `VFS: Replacing file with directory at segment "${segment}" while ensuring directory "${path}"`,
          );
  ...
  async truncateFile(path: string): Promise<void> {
    ...
    try {
      ...
      if (staleWriter) {
        ...
        try {
          await staleWriter.writer?.close();
        } catch {}
      }
  ...
  async readFile(path: string, offset: number, length: number): Promise<Uint8Array> {
    Logger.verbose(
      LogCategory.SYSTEM,
      `OPFS: readFile("${path}", offset=${offset}, length=${length}) starting`,
    );
    ...
    if (writerEntry) {
      ...
      if (buf.length > 0 && offset >= bufStart && offset + length <= bufStart + buf.length) {
        ...
        Logger.verbose(
          LogCategory.SYSTEM,
          `OPFS: readFile("${path}") served from writer memoryBuffer, ${length} bytes`,
        );
      ...
      try {
        await writerEntry.writer?.close();
      } catch {}
      Logger.verbose(
        LogCategory.SYSTEM,
        `OPFS: readFile("${path}") flushed and closed active writer before read`,
      );
    }
    ...
    if (syncResult !== null) {
      Logger.verbose(
        LogCategory.SYSTEM,
        `OPFS: readFile("${path}") served from sync handle, ${syncResult.length} bytes`,
      );
      return syncResult;
    }
    ...
    if (syncHandle) {
      ...
      Logger.verbose(
        LogCategory.SYSTEM,
        `OPFS: readFile("${path}") opened sync handle, read ${bytesRead} bytes`,
      );
      return buffer.subarray(0, bytesRead);
    }
    ...
    try {
      ...
      Logger.error(
        LogCategory.SYSTEM,
        `OPFS: readFile("${path}", offset=${offset}, length=${length}) failed: ${e}`,
      );
      throw e;
    }
  }
  ...
  async writeFile(path: string, offset: number, data: Uint8Array): Promise<number> {
    try {
      ...
      Logger.error(
        LogCategory.SYSTEM,
        `OPFS: writeFile("${path}", offset=${offset}, size=${data.length}) failed: ${e}`,
      );
      throw e;
    }
  }
    ...
    const run = (async () => {
      ...
      if (cacheEntry.flushInFlight) {
        try {
          ...
          Logger.warn(
            LogCategory.SYSTEM,
            `OPFS: flushFile("${path}") in-flight buffer flush failed: ${e}`,
          );
        }
      }
      ...
      try {
        if (cacheEntry.writer) {
          ...
          if (pending.length > 0) {
            ...
            await cacheEntry.writer.write(
              (pending.length === pending.buffer.byteLength
                ? pending.buffer
                : new Uint8Array(pending)) as unknown as FileSystemWriteChunkType,
            );
          }
          await cacheEntry.queue;
  ...
  async flushAll(): Promise<void> {
    ...
    for (const p of pending) {
      try {
        ...
        /* commit error already logged in flushFile */
      }
    }
  ...
  private async getFileHandle(path: string, create: boolean): Promise<FileSystemFileHandle> {
    ...
    try {
      ...
      Logger.verbose(
        LogCategory.SYSTEM,
        `OPFS: getFileHandle got directory handle in ${dirTime.toFixed(2)}ms, filename="${name}"`,
      );
      ...
      Logger.verbose(
        LogCategory.SYSTEM,
        `OPFS: getFileHandle got file handle in ${fileTime.toFixed(2)}ms`,
      );
      return fileHandle;
    } catch (e) {
      Logger.error(
        LogCategory.SYSTEM,
        `OPFS: getFileHandle("${path}", create=${create}) failed: ${e}`,
      );
      throw e;
    }
  }
  ...
  private async getDirectoryHandle(
    path: string,
    create: boolean,
  ): Promise<{ dirHandle: FileSystemDirectoryHandle; name: string }> {
    Logger.verbose(
      LogCategory.SYSTEM,
      `OPFS: getDirectoryHandle("${path}", create=${create}) starting`,
    );
    if (!this.root) {
      Logger.error(
        LogCategory.SYSTEM,
        `OPFS: getDirectoryHandle("${path}") failed: OPFS not initialized`,
      );
      throw new Error('OPFS not initialized');
    }
    const segments = toOpfsSegments(path);
    if (segments.length === 0) {
      Logger.error(
        LogCategory.SYSTEM,
        `OPFS: getDirectoryHandle("${path}") failed: Invalid path (empty segments)`,
      );
      throw new Error('Invalid path');
    }
    Logger.verbose(
      LogCategory.SYSTEM,
      `OPFS: getDirectoryHandle path segments: [${segments.join(', ')}]`,
    );
    ...
    for (let i = 0; i < segments.length - 1; i++) {
      ...
      try {
        ...
        Logger.verbose(
          LogCategory.SYSTEM,
          `OPFS: getDirectoryHandle got segment "${segment}" (${i + 1}/${segments.length - 1}) in ${segmentTime.toFixed(2)}ms`,
        );
      } catch (e: any) {
        if (e.name === 'TypeMismatchError') {
          ...
          if (create) {
            Logger.warn(
              LogCategory.SYSTEM,
              `OPFS: Replacing file with directory at segment "${segment}" for path "${path}"`,
            );
            await dir.removeEntry(segment);
            dir = await dir.getDirectoryHandle(segment, { create: true });
            const segmentTime = performance.now() - segmentStart;
            Logger.verbose(
              LogCategory.SYSTEM,
              `OPFS: getDirectoryHandle replaced and got segment "${segment}" in ${segmentTime.toFixed(2)}ms`,
            );
          } else {
            Logger.error(
              LogCategory.SYSTEM,
              `OPFS: getDirectoryHandle("${path}") failed at segment "${segment}": TypeMismatchError and create=false`,
            );
            throw e;
          }
        } else {
          Logger.error(
            LogCategory.SYSTEM,
            `OPFS: getDirectoryHandle("${path}") failed at segment "${segment}": ${e}`,
          );
          throw e;
        }
      }
    ...
    Logger.verbose(
      LogCategory.SYSTEM,
      `OPFS: getDirectoryHandle("${path}") completed, returning directory with filename="${fileName}"`,
    );
  ...
  private async scanDirectory(dir: FileSystemDirectoryHandle, prefix: string[]): Promise<void> {
    ...
    const dirEntries = (
      dir as any as { entries(): AsyncIterableIterator<[string, FileSystemHandle]> }
    ).entries();
    for await (const [name, handle] of dirEntries) {
      ...
      if (handle.kind === 'file') {
        ...
        this.entries.set(toKey(vfsPath), {
          size: file.size,
          kind: 'file',
          path: normalizePath(vfsPath),
        });
      } else {
