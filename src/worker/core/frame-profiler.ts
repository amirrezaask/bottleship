...
// A timing-only window large enough for a two-minute 60 FPS scenario plus
// modest refresh-rate variance. Detailed category samples retain their smaller
// diagnostic window; this adds only 64 KiB and is allocated once.
const FRAME_TIMING_WINDOW_SIZE = 8_192;
...
const PARK_THUNKS = new Set<string>([
  ...
  'user32:peekmessagea',
  'user32:waitmessage',
  'ddraw:idirectdrawsurface7_flip',
  'ddraw:idirectdrawsurface_flip',
...
export class FrameProfiler {
  ...
  private readonly frameTimesMs = new Float64Array(FRAME_TIMING_WINDOW_SIZE);
  private frameTimeIndex = 0;
  private frameTimeCount = 0;
  private totalSampleCount = 0;
  private counterOverflow = false;
  ...
  /** Enable only the bounded frame-time ring, without category/thunk capture. */
  ...
  reset(): void {
    ...
    this.frameTimesMs.fill(0);
    this.totalSampleCount = 0;
    this.counterOverflow = false;
  ...
  markFrame(source: string): void {
    if (!this.enabled && !this.timingEnabled) return;
    const now = performance.now();
    ...
    if (!this.enabled) {
      ...
      this.lastFrameTime = now;
      if (!(frameMs > 0)) return;
      this.frameTimesMs[this.frameTimeIndex] = frameMs;
      this.frameTimeIndex = (this.frameTimeIndex + 1) % this.frameTimesMs.length;
      this.frameTimeCount = Math.min(this.frameTimeCount + 1, this.frameTimesMs.length);
      if (this.totalSampleCount === Number.MAX_SAFE_INTEGER) this.counterOverflow = true;
      else this.totalSampleCount++;
      return;
    }
    ...
    if (!isSpike) {
      this.rollingAvgFrameMs =
        this.rollingAvgFrameMs * (1 - this.ROLLING_AVG_ALPHA) + frameMs * this.ROLLING_AVG_ALPHA;
    }
  ...
  private accumulateStall(c: BadFrameCapture): void {
    ...
    else if (topName && topMs >= ms * 0.3) cause = `thunk:${topName}`;
    ...
    if (!b) {
      b = { count: 0, sumMs: 0, maxMs: 0, sumExcessMs: 0 };
      this.stallStats.set(sig, b);
    }
    b.count++;
    b.sumMs += ms;
  ...
  getStallReport() {
    const classes = Array.from(this.stallStats.entries())
      .map(([signature, b]) => ({
        signature,
        count: b.count,
    ...
    const worst = this.worstStalls.map((c) => ({
      ...
      io:
        (c.thunkAggregates['kernel32:ReadFile']?.count ?? 0) +
        (c.thunkAggregates['kernel32:SetFilePointer']?.count ?? 0),
  ...
  getSnapshot(maxSamples: number = 60): FrameStatsSnapshot {
    if (!this.enabled || this.sampleCount === 0) {
      return {
        ...
        droppedSamples: Math.max(0, this.totalSampleCount - this.sampleCount),
        counterOverflow: this.counterOverflow,
  ...
  /**
   * Return the bounded timing-only window used by qualification tooling.
   * Keeping it separate avoids retaining category arrays and thunk maps for
   * thousands of frames while preserving exact retained-frame percentiles.
   */
  getTimingSnapshot(): FrameTimingSnapshot {
    const frameTimesMs: number[] = [];
    const start =
      (this.frameTimeIndex - this.frameTimeCount + this.frameTimesMs.length) %
      this.frameTimesMs.length;
    for (let index = 0; index < this.frameTimeCount; index++) {
      frameTimesMs.push(this.frameTimesMs[(start + index) % this.frameTimesMs.length]);
    }
    return {
      enabled: this.enabled,
      source: this.currentSource,
      totalSampleCount: this.totalSampleCount,
      retainedSampleCount: this.frameTimeCount,
      droppedSamples: Math.max(0, this.totalSampleCount - this.frameTimeCount),
      counterOverflow: this.counterOverflow,
      windowSize: this.frameTimesMs.length,
      frameTimesMs,
    };
  }
