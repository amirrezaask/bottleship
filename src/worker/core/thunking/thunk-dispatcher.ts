...
import {
  ThunkMemoryManager,
  ThunkMemoryRegions,
  SEH_CATCH_COMPLETION_FUNCID,
} from './thunk-memory-manager';
import {
...
import { bootMark } from '../boot-timer';
...
import { SEH_SCRATCH_LAYOUT, EH3_FILTER_CTX_LAYOUT, SEH_FRAME_LIST_MAX } from './seh-layout';
...
export class ThunkDispatcher {
  ...
  public dbgFastPathRec: Map<
    number,
    { count: number; lastRet: number; lastCaller: number }
  > | null = null;
  ...
  private pendingRegistrations: Map<
    string,
    { impl: ThunkImplementation; dllName: string; functionName: string }
  > = new Map();
  private pendingFastPathRegistrations: Map<
    string,
    { impl: FastPathImplementation; dllName: string; functionName: string; trivial?: boolean }
  > = new Map();
  private pendingWriteBufRegistrations: Map<
    string,
    {
      ...
      ptrDeref?: boolean;
      floatCount?: number;
      shaderConstant?: boolean;
      coalesceArgMask?: number;
      shadowSpec?: ShadowTrampolineSpec;
      barrier?: boolean;
      structCapture?: { ptrArgIndex: number; payloadDwords: number };
      upDraw?: boolean;
      ownerDisarm?: boolean;
    }
  > = new Map();
  private pendingConstStubRegistrations: Map<
    string,
    { dllName: string; functionName: string; value: number; popBytes: number }
  > = new Map();
    ...
    (globalThis as any).__guestBtLite = (): string => {
      try {
        ...
        return bt.frames
          ...
          .map((f) =>
            f.moduleName
              ? `${f.moduleName}+0x${f.moduleOffset.toString(16)}`
              : `0x${f.retAddr.toString(16)}`,
          )
          .join(' <- ');
      } catch {
        return '';
  ...
  private isDataViewValid(): boolean {
    ...
    try {
      return (
        this.cachedMem8.byteLength > 0 && this.cachedDataView.buffer === this.cachedMem8.buffer
      );
    } catch {
  ...
  public updateMemoryCache(): void {
    ...
    const mem8 = this.getMemory
      ? this.getMemory()
      : this.v86.mem8 || (this.v86.v86 && this.v86.v86.cpu.mem8);
    if (mem8 && mem8.byteLength > 0) {
      ...
      if (
        !this.cachedDataView ||
        this.cachedDataView.buffer !== mem8.buffer ||
        (this.cachedMem8 && this.cachedMem8.byteLength === 0)
      ) {
  ...
  initializeCallbackManager(
    ...
    thunkMemoryManager?: ThunkMemoryManager,
  ): void {
   ...
   */
  public getGuestCallStack(
    esp?: number,
    scanBytes: number = 0x800,
    maxFrames: number = 48,
  ): {
    esp: number;
    lastThunk: string;
    ...
    const view =
      this.isDataViewValid() && this.cachedDataView
        ? this.cachedDataView
  ...
  private wbufCoalesceLatest(
    ...
    const k3 = mask & 4 ? mem32[w + 2] >>> 0 : 0;
    const slot = this.wbufCoalesceSlot(k0, k1, k2, k3);
    return this.wbufCoalesceUsed[slot] === this.wbufCoalesceEpoch
      ? this.wbufCoalesceOffset[slot]
      : -1;
  }

  private getWbufEntryStride(
    mem32: Uint32Array,
    dataBase: number,
    offset: number,
    argCount: number,
  ...
  private buildWbufCoalesceIndex(
    mem32: Uint32Array,
    dataBase: number,
    start: number,
    head: number,
  ...
  private drainWriteBuffer(): void {
    ...
    const coalescing =
      this.wbufCoalescingEnabled && this.buildWbufCoalesceIndex(mem32, dataBase, offset, head);
    while (offset < head) {
      const funcId = mem32[(dataBase + offset) >> 2];
      if (funcId > 0 && funcId < MAX_THUNK_ID) {
        ...
        if (handler && argCount > 0) {
          ...
          if (
            coalesceMask &&
            this.wbufCoalesceLatest(mem32, dataBase + offset + 4, funcId, coalesceMask, segment) !==
              offset
          ) {
          ...
          Logger.warn(
            LogCategory.THUNK,
            `drainWriteBuffer: unregistered funcId ${funcId} (argCount=${argCount}) in ring at offset ${offset}`,
          );
        ...
        Logger.warn(
          LogCategory.THUNK,
          `drainWriteBuffer: unexpected funcId 0x${funcId.toString(16)} at offset ${offset}`,
        );
        offset = head;
        break;
      }
  ...
  private handlePortWrite(functionId: number): void {
    ...
    // One capture boundary for both fast and slow JS thunk paths. Promise-backed
    // thunks enter here once, so completion cannot double-count them. Native
    // hypercalls and tier-0 write-buffer trampolines bypass this method; the
    // collector advertises dispatcher-only coverage for that reason.
    let callProfileName: string | null = null;
    if (gameBoxCallProfile.isRunning() && functionId > 0 && functionId < MAX_THUNK_ID) {
      let name = this.namesTable[functionId];
      if (!name) {
        const stub = this.thunkGenerator?.getStubById(functionId);
        name = stub ? `${stub.dllName}:${stub.functionName}` : `id:${functionId}`;
      }
      callProfileName = name;
      gameBoxCallProfile.record(name);
    }
    ...
    if (this.hcRingEnabled) {
      ...
      if (this.cachedDataView && this.isDataViewValid()) {
        if (esp) {
          const sp = esp[4] >>> 0;
          if (sp + 4 <= this.memLength) caller = this.cachedDataView.getUint32(sp, true) >>> 0;
        }
        if (this.hcWatchAddr && this.hcWatchAddr + 4 <= this.memLength)
          head = this.cachedDataView.getUint32(this.hcWatchAddr, true) >>> 0;
      }
    ...
    if (functionId > 0 && functionId < MAX_THUNK_ID && this.writeBufHandlerTable[functionId]) {
      ...
      if (this.wbufMissLogCount < 5) {
        ...
        if (stub && this.cachedMem8) {
          ...
          const bytes = Array.from(m.slice(a, a + 16))
    ...
    if (functionId > 0 && functionId < MAX_THUNK_ID) {
      ...
      if (fastImpl) {
        ...
        if (doProfile) {
          ...
          let res: number | null | undefined;
          if (callProfileName !== null) {
            const callStart = performance.now();
            try {
              res = fastImpl(cpu, this.cachedMem8, this.cachedMem32!, this.cachedDataView!);
            } catch (error) {
              gameBoxCallProfile.recordCompletion(
                callProfileName,
          ...
          if (res !== null) {
            ...
            const cleanupBytes =
              cachedCleanup >= 0 ? cachedCleanup : (fastArgCount >= 0 ? fastArgCount : 0) * 4;
        ...
        this._handlePortWriteSlow(functionId, true, callProfileName);
        return;
      }
    }

    // All non-fast-path calls go through the slow path
    this._handlePortWriteSlow(functionId, false, callProfileName);
  }
  ...
  private _recordFastPath(functionId: number, res: number | null | undefined | object): void {
    ...
    if (!e) {
      e = { count: 0, lastRet: 0, lastCaller: 0 };
      rec.set(functionId, e);
    }
    e.count++;
  ...
  dbgFastPathStart(): void {
    this.dbgFastPathRec = new Map();
  }
  dbgFastPathDump(): Array<{ name: string; count: number; lastRet: string; lastCaller: string }> {
    ...
    for (const [id, e] of rec.entries()) {
      out.push({
        name: this.namesTable[id] || `id_0x${id.toString(16)}`,
        count: e.count,
        lastRet: `0x${(e.lastRet >>> 0).toString(16)}`,
        lastCaller: `0x${(e.lastCaller >>> 0).toString(16)}`,
      });
    }
    out.sort((a, b) => b.count - a.count);
    return out;
  }
  dbgFastPathStop(): void {
    this.dbgFastPathRec = null;
  }
   ...
   */
  private _handlePortWriteSlow(
    functionId: number,
    schedulerEnterAlreadyCalled: boolean,
    callProfileName: string | null = null,
  ): void {
    ...
    if (system.isPaused) {
      Logger.warn(
        LogCategory.THUNK,
        `Thunk call 0x${functionId.toString(16)} during PAUSE! Stopping CPU.`,
      );
      try {
        this.v86.stop();
      } catch {}
    ...
    if (system.isExiting) {
      Logger.verbose(
        LogCategory.THUNK,
        `Ignoring thunk call during exit: 0x${functionId.toString(16)}`,
      );
      try {
        this.v86.stop();
      } catch {}
    ...
    if (this.isWaitingForEipDump) {
      ...
      if (this.cachedMem8 && eipValue > 0 && eipValue < this.memLength) {
        ...
        const codeBytes = Array.from(
          this.cachedMem8.slice(start, Math.min(this.memLength, start + 15)),
        );
        bytesInfo = ` | Bytes around EIP: ${codeBytes.map((b: any) => (b as number).toString(16).padStart(2, '0')).join(' ')}`;
      }
      Logger.error(
        LogCategory.SYSTEM,
        `[DIAG] Faulting EIP captured: 0x${eipValue.toString(16)}${bytesInfo}`,
      );
      ...
      if (stub) {
        ...
        Logger.error(
          LogCategory.SYSTEM,
          `   Faulting EIP is inside thunk stub ${stub.dllName}:${stub.functionName} +0x${offset.toString(16)}`,
        );
      } else {
        if (eipValue >= 0x80000 && eipValue < 0x100000) {
          Logger.error(
            LogCategory.SYSTEM,
            `Execution escaped to STACK at 0x${eipValue.toString(16)}!`,
          );
          if (this.cachedMem8 && this.isDataViewValid()) {
            const bytes = Array.from(
              this.cachedMem8.slice(eipValue, Math.min(this.memLength, eipValue + 16)),
            );
            Logger.error(
              LogCategory.SYSTEM,
              `   Stack bytes at EIP: ${bytes.map((b) => b.toString(16).padStart(2, '0')).join(' ')}`,
            );
          }
    ...
    if ((functionId & 0x80000000) !== 0) {
      if (this._callbackManager) {
        ...
        if (preEsp !== postEsp) {
          ...
          if (
            this.cachedMem8 &&
            this.isDataViewValid() &&
            postEsp >= 0 &&
            postEsp + 8 <= this.memLength
          ) {
    ...
    if (functionId === 0xffffffff || functionId === 0) {
      const eipVal =
        (this.cachedIpRaw ? this.cachedIpRaw[0] : (cpu.instruction_pointer?.[0] ?? 0)) >>> 0;
      Logger.warn(
        LogCategory.THUNK,
        `Thunk port write 0x${functionId.toString(16)} (invalid ID, ignored) context: EIP=0x${eipVal.toString(16)}`,
      );
      this.boundaryKind = ThunkBoundaryKind.GUEST_CODE;
      this.boundaryCleanup = 0;
      this.notifySchedulerBoundary(cpu);
      ...
      if (stubStart >= 0) {
        ...
        if (b[stubStart] === 0xb8 && b[stubStart + 5] === 0xba && b[stubStart + 10] === 0xef) {
          ...
          if (functionId !== stubId) {
            Logger.error(
              LogCategory.THUNK,
              `?? OFFSET +5 CALL DETECTED! Correcting ID ${functionId} -> ${stubId}`,
            );
            functionId = stubId;
          }
        }
    ...
    if ((this.thunkCount & 0x3f) === 0) {
      ...
      const isTimePoll =
        thunkName === 'winmm:timeGetTime' ||
        thunkName === 'kernel32:GetTickCount' ||
        thunkName === 'kernel32:QueryPerformanceCounter';
      if (isTimePoll) {
        ...
        if (current && this.thunkCallCounts[functionId] % 4 === 0) {
          ...
          if (
            sched.insnQuantumFraction(current, 0.5) &&
            sched.hasOtherRunnableThreads(current.id)
          ) {
            sched.requestSwitch();
          }
        }
    ...
    if (
      ...
      this.isDataViewValid() &&
      espAtEntry >= 0 &&
      espAtEntry + 4 <= this.memLength
    ) {
      const retAddrAtEntry = this.cachedDataView!.getUint32(espAtEntry, true) >>> 0;
      if (retAddrAtEntry >= GUEST_CODE_START) {
        const mod =
          System.getInstance().process?.moduleRegistry.getModuleContainingAddress(retAddrAtEntry);
    ...
    const sampleProfiler =
      profilerEnabled && (this.profilerSampleCounter++ & this.profilerSampleMask) === 0;
    ...
    if (!impl) {
      impl =
        this._trySameNameLookup(functionId) ||
        this._tryDllForward(functionId) ||
        this._tryPendingRegistration(functionId);
    }
    ...
    if (argCount < 0) {
      Logger.warn(
        LogCategory.THUNK,
        `?? STACK DRIFT RISK: ${thunkName} (id=0x${functionId.toString(16)}) has no argCount, using ${DEFAULT_ARGS_COUNT}`,
      );
    ...
    frameProfiler.markThunkStart();
    const thunkStart = frameProfiler.startTimer();
    // PERF: Capture wall-clock time for virtual time compensation and the
    // opt-in GameBox call profile. The profile gate is carried from the
    // dispatcher boundary so disabled calls do not perform a clock read.
    ...
    const implWallStart = callProfileName !== null ? performance.now() : 0;
    try {
      ...
      ctx.eflags = (cpu as any)['get_eflags']
        ? (cpu as any)['get_eflags']()
        : (this.cachedFlags || cpu.flags)[0];
      ...
      const censusCaller =
        this.cachedDataView && this.isDataViewValid() && espAtEntry < this.memLength - 4
          ? this.cachedDataView.getUint32(espAtEntry, true) >>> 0
          : 0;
    ...
    try {
      ...
      if (this.cachedMem32 && (esp & 3) === 0) {
        ...
        if (index < this.cachedMem32.length) {
          ...
          if (
            !allowStubPoolRet &&
            this.callbackStubPoolBase !== 0 &&
            retAddr >= this.callbackStubPoolBase &&
            retAddr < this.callbackStubPoolEnd
          ) {
            return false;
          }
  ...
  private validateReturnAddr(
    mem: Uint8Array,
    esp: number,
    label: string,
    allowStubPoolRet = false,
  ): boolean {
    if (esp < 0 || esp + 4 > mem.length) {
      Logger.error(
        LogCategory.THUNK,
        `${label}: invalid ESP 0x${esp.toString(16)} (mem size=0x${mem.length.toString(16)})`,
      );
      return false;
    }
    ...
    if (
      returnAddr < 0x1000 ||
      returnAddr >= mem.length ||
      returnAddr === 0xcccccccc ||
      returnAddr === 0xdeadbeef ||
      returnAddr === 0
    ) {
      Logger.error(
        LogCategory.THUNK,
        `${label}: invalid returnAddr 0x${returnAddr.toString(16)} (mem size=0x${mem.length.toString(16)})`,
      );
      return false;
    }
    ...
    if (mem[returnAddr] === 0 && mem[returnAddr + 1] === 0) {
      Logger.warn(
        LogCategory.THUNK,
        `${label}: returnAddr 0x${returnAddr.toString(16)} points to zeroed memory (00 00...)`,
      );
    }
  ...
  private _handleSyncResult(
    ...
    if (typeof result === 'object' && result !== null && (result as ThunkResult).terminated) {
      ...
      if (!hasOtherThreads || System.getInstance().isExiting) {
        ...
        Logger.log(
          LogCategory.THUNK,
          `No runnable threads or process exiting - stopping v86 (isExiting=${System.getInstance().isExiting})`,
        );
        if (this.v86?.stop) {
          this.v86.stop();
        }
    ...
    if (
      ...
      result !== null &&
      (result as ThunkResult).suspendedForCallback
    ) {
      ...
      const suspendErrCleanup = this.resolveThunkCleanup(
        id,
        argCount,
        `sync:suspended-err:${name}`,
      );
    ...
    const skipStackCheck =
      typeof result === 'object' && result !== null && (result as ThunkResult).skipStackCheck;
    const currentEsp = reg32[4];
    if (!skipStackCheck && currentEsp !== espAtEntry && !cpu.is_jumping) {
      Logger.warn(
        LogCategory.THUNK,
        `?? STACK DRIFT: ${name} changed ESP: 0x${espAtEntry.toString(16)} -> 0x${currentEsp.toString(16)}`,
      );
    }
    ...
    if (
      mem8 &&
      !skipStackCheck &&
      !this.validateReturnAddr(mem8, espAtEntry, `Sync thunk ${name}`, allowStubSync)
    ) {
      Logger.error(
        LogCategory.THUNK,
    ...
    if (thunkRes.stackCleanup !== undefined && !thunkRes.skipStackCheck) {
      ...
      if (finalCleanup !== stubCleanup) {
        ...
        if (this.isDataViewValid()) {
          ...
          Logger.error(
            LogCategory.THUNK,
            `Stack-based drift fix failed for ${name}: DataView detached`,
          );
        }
      }
  ...
  private parkThreadAsync(
    ...
    if (returnAddr !== undefined) {
      if (
        this.thunkGeneratorBase !== 0 &&
        returnAddr >= this.thunkGeneratorBase &&
        returnAddr < this.thunkGeneratorEnd
      ) {
        Logger.error(
          LogCategory.THUNK,
    ...
    if (scheduler.markThreadAsyncParked(tid, cpu)) {
      ...
      (scheduler as any).traceAsyncRestore?.(
        'parkThreadAsync',
        cpu,
  ...
  private _onAsyncComplete(
    ...
    this.recordAsyncPark(name, info.threadId ?? 0, completionNow - info.startTime);
    if (callProfileName !== null) {
      gameBoxCallProfile.recordCompletion(
        callProfileName,
        completionNow - callProfileStart,
        'async',
      );
    }
    ...
    const dllInits =
      typeof res === 'object' && res !== null ? (res as ThunkResult).dllInits : undefined;
    const startCallbackChain =
      typeof res === 'object' && res !== null ? (res as ThunkResult).startCallbackChain : undefined;
    ...
    if (info.threadId !== undefined) {
      (this.ensureScheduler() as any).traceTimerEvent?.(
        info.threadId,
        `restoreQueued ${name} ret=0x${((info.returnAddr ?? 0) >>> 0).toString(16)}`,
      );
      (this.ensureScheduler() as any).traceAsyncRestore?.(
        'onAsyncComplete',
        null,
  ...
  private _onAsyncError(
    ...
    const cleanupBytes = this.resolveThunkCleanup(
      info.functionId,
      defaultArgCount,
      `async_error:${name}`,
    );
   ...
   */
  tryApplyPendingAsyncRestoreAtSafePoint(
    cpu: any,
    source: string = 'onPollAsyncRestores',
  ): boolean {
    if (this.pendingAsyncRestores.length === 0) return false;
    ...
    for (let i = this.pendingAsyncRestores.length - 1; i >= 0; i--) {
      ...
      if (cls.ok) {
        pending.transientValidationRetries = 0;
        continue;
      }
      if (cls.transient) {
        ...
        const n = (pending.transientValidationRetries =
          (pending.transientValidationRetries ?? 0) + 1);
    ...
    if (currentTid !== null) {
      ...
      if (idx >= 0 && this.canApplyCurrentThreadRestore(cpu, this.pendingAsyncRestores[idx])) {
        ...
        (scheduler as any).traceTimerEvent?.(
          currentTid,
   ...
   */
  private classifyPendingAsyncRestore(
    pending: PendingAsyncRestore,
    source: string,
  ): { ok: boolean; transient: boolean; reason: string } {
  ...
  private rejectPendingAsyncRestore(
    pending: PendingAsyncRestore,
    cpu: any,
    source: string,
    reason: string,
  ...
  private canApplyCurrentThreadRestore(cpu: any, pending: PendingAsyncRestore): boolean {
    ...
    const hasInFlightCallbacks =
      typeof (cbMgr as any).hasInFlightCallbacks === 'function'
  ...
  private isInAsyncThunkStubReturnPath(eip: number): boolean {
    if (
      this.thunkGeneratorBase === 0 ||
      eip < this.thunkGeneratorBase ||
      eip >= this.thunkGeneratorEnd
    ) {
      return false;
    }
  ...
  private applyPendingAsyncRestoreAtSafePoint(pending: PendingAsyncRestore, cpu: any): void {
    ...
    if (startCallbackChain) {
      ...
      if (!this.cachedMem8) {
        Logger.error(
          LogCategory.THUNK,
          `startCallbackChain (${completionName}): memory unavailable`,
        );
        this._restoreAsyncContext(
          info,
          cpu,
          returnValue,
          cleanupBytes,
      ...
      let started = false;
      try {
        started = startCallbackChain({
          esp: info.esp ?? 0,
          returnAddr: (info.returnAddr ?? 0) >>> 0,
          mem: this.cachedMem8,
        });
      } catch (e) {
      ...
      if (!started) {
        ...
        Logger.warn(
          LogCategory.THUNK,
          `startCallbackChain (${completionName}) did not start — falling back to plain return`,
        );
        this._restoreAsyncContext(
          info,
          cpu,
    ...
    const isLoadLibrary =
      baseName === 'LoadLibraryExW' ||
      baseName === 'LoadLibraryExA' ||
      baseName === 'LoadLibraryW' ||
      baseName === 'LoadLibraryA';
    ...
    if (dllInits && dllInits.length > 0 && isLoadLibrary && this._callbackManager) {
      ...
      if (!dllMainFrameId) {
        Logger.error(
          LogCategory.THUNK,
          `[DllMain] Failed to save suspended thunk frame for ${completionName}`,
        );
        this._restoreAsyncContext(
          info,
          cpu,
   ...
   */
  static reconcileAsyncRestoreEsp(
    parkEsp: number,
    cleanupBytes: number,
    liveEsp: number,
  ): { esp: number; mismatch: boolean } {
  ...
  private _restoreAsyncContext(
    ...
    if (!this.validateReturnAddr(mem8, esp, `Async thunk return (${name})`)) {
      Logger.error(
        LogCategory.THUNK,
        `Stopping due to invalid return address (async). ` +
    ...
    const scheduler = this.ensureScheduler();
    if (
      !scheduler.applyAsyncRestoreCpuState(
        cpu,
        eipLinear,
        finalEsp,
        returnValue >>> 0,
    ...
    Logger.verbose(
      LogCategory.THUNK,
      `Async thunk completed: ${name}, EIP=0x${eipLinear.toString(16)}, ESP=0x${cpu.reg32[4].toString(16)}`,
    );
    ...
    if (System.getInstance().isPaused) {
      Logger.verbose(
        LogCategory.THUNK,
        `Async thunk ${name} context restored during pause, skipping v86.run()`,
      );
      return;
    }
  ...
  registerFastPath(
    ...
    const key = `${dllName}:${functionName}`.toLowerCase();
    this.pendingFastPathRegistrations.set(key, {
      impl,
      dllName,
      functionName,
    ...
    if (matchingStubs.length === 0) {
      Logger.verbose(
        LogCategory.THUNK,
        `Fast-path stub not found for ${dllName}:${functionName}, registration deferred`,
      );
      return;
    }
    for (const stub of matchingStubs) {
      if (stub.functionId < MAX_THUNK_ID) {
        ...
        Logger.verbose(
          LogCategory.THUNK,
          `Registered fast-path [${stub.functionId}] ${dllName}:${functionName}${options?.trivial ? ' (trivial)' : ''}`,
        );
      }
    }
  }
  ...
  registerWriteBufferFunction(
    ...
    const readBack = Array.from(mem8.slice(stubAddr, stubAddr + 16))
      .map((b) => b.toString(16).padStart(2, '0'))
      .join(' ');
  ...
  registerShadowedWriteBufferFunction(
    ...
    if (this.shadowOwnerGlobal === 0)
      this.shadowOwnerGlobal = this.thunkMemoryManager.allocShadowOwnerGlobal();

    const h = writeShadowTrampoline(
      this.thunkMemoryManager.stubAllocator,
      this.getMemory,
      this.writeBufControlAddr,
      this.writeBufDataBase,
      this.writeBufCapacity,
      this.shadowOwnerGlobal,
      spec,
    );
    ...
    this.registerWriteBufferFunction(dllName, funcName, argCount, handler, true, coalesceArgMask, {
      trampolineOverride: h.trampAddr,
      shadowSpec: spec,
    });

    this.shadowHandles.set(key, {
      trampAddr: h.trampAddr,
      shadowBase: h.shadowBase,
      slotCount: h.slotCount,
      sentinel: h.sentinel,
      skipCounterAddr: h.skipCounterAddr,
    });
    Logger.log(
      LogCategory.THUNK,
  ...
  registerOwnerDisarmWriteBufferFunction(
    ...
    if (this.shadowOwnerGlobal === 0) {
      ...
      this.registerWriteBufferFunction(
        ...
        true,
        coalesceArgMask,
        { barrier: opts?.barrier, ownerDisarm: true },
      );
      return;
    }
  ...
  writeShadowSlot(dllName: string, funcName: string, slot: number, value: number): void {
    ...
    const mem = this.getMemory();
    new DataView(mem.buffer, mem.byteOffset, mem.byteLength).setInt32(
      h.shadowBase + slot * 4,
      value | 0,
      true,
    );
  ...
  getThunkStubInfo(
    dllName: string,
    funcName: string,
  ): { functionId: number; address: number } | null {
    const stub = this.findStubsByName(dllName, funcName)[0];
  ...
  registerPtrDerefWriteBufferFunction(
    ...
    const trampolineIdx = floatCount === 3 ? 16 + (isStdcall ? 0 : 1) : 18 + (isStdcall ? 0 : 1);
  ...
  registerStructCaptureWriteBufferFunction(
    ...
    if (id < 0) {
      Logger.verbose(
        LogCategory.THUNK,
        `StructCapture stub not found for ${dllName}:${funcName}, registration deferred`,
      );
      return;
    }
    this.writeBufHandlerTable[id] = handler;
   ...
   */
  registerUpDrawWriteBufferFunction(
    dllName: string,
    funcName: string,
    handler: WriteBufHandler,
  ): void {
    ...
    if (id < 0) {
      Logger.verbose(
        LogCategory.THUNK,
        `UpDraw stub not found for ${dllName}:${funcName}, registration deferred`,
      );
      return;
    }
    this.writeBufHandlerTable[id] = handler;
  ...
  registerConstantReturnStub(
    ...
    const key = `${dllName}:${funcName}`.toLowerCase();
    this.pendingConstStubRegistrations.set(key, {
      dllName,
      functionName: funcName,
      value,
      popBytes,
  ...
  getThunkCleanupAtEip(eip: number): number {
    ...
    if (
      this.thunkGeneratorBase === 0 ||
      addr < this.thunkGeneratorBase ||
      addr >= this.thunkGeneratorEnd
    ) {
      return -1;
    }
  ...
  private recordShadowStackGuard(
    ...
    if (this.shadowStackRingCount < ThunkDispatcher.SHADOW_STACK_RING_SIZE)
      this.shadowStackRingCount++;
  }
  ...
  private _registerSehTransientRanges(): void {
    ...
    if (this.sehDispatchStubAddress) {
      sched.registerTransientExecRange(
        'seh_dispatch_stub',
        this.sehDispatchStubAddress >>> 0,
        (this.sehDispatchStubAddress + 0x100) >>> 0,
      );
    }
    if (this.sehFilterStubAddress) {
      sched.registerTransientExecRange(
        'seh_filter_stub',
        this.sehFilterStubAddress >>> 0,
        (this.sehFilterStubAddress + 0x100) >>> 0,
      );
    }
  ...
  private checkEspSanity(esp: number, thunkName: string): void {
    const base = this.curStackBase,
      top = this.curStackTop;
    ...
    if (this.sehStackTop > 0 && esp >= (this.sehStackBase - 0x100) >>> 0 && esp < this.sehStackTop)
      return;
  ...
  private checkEbpSanity(ebp: number, thunkName: string): void {
    ...
    if (this.sehStackTop > 0 && ebp >= (this.sehStackBase - 0x100) >>> 0 && ebp < this.sehStackTop)
      return;
  ...
  getLastWinApiCallsRich(
    count: number = 50,
  ): Array<{ id: number; name: string; esp: number; retAddrBefore: number; stackHash: number }> {
  ...
  getLastHypercalls(count: number = 48): string[] {
    ...
    for (let i = n; i >= 1; i--) {
      ...
      const name =
        (id > 0 && id < this.namesTable.length ? this.namesTable[id] : null) || `id_${id >>> 0}`;
      const h = this.hcRingHead[p] >>> 0;
      out.push(
        `T${this.hcRingThread[p]} ${name} <-0x${(this.hcRingCaller[p] >>> 0).toString(16)}${this.hcWatchAddr ? ` head=0x${h.toString(16)}${h === 0 ? ' *TORN*' : ''}` : ''}`,
      );
    }
    return out;
  }
  ...
  maybeScheduleChecksumValidation(force: boolean = false): void {
    ...
    thunkChecksumManager
      .validateThunkRegion(mem8, 'heartbeat')
      .then((valid) => {
        if (!valid) {
          Logger.error(
            LogCategory.SYSTEM,
            'Thunk checksum validation failed during heartbeat - halting emulator',
          );
          System.getInstance().isExiting = true;
        }
      })
   ...
   */
  private isAddressSafeForDeferredWrite(
    addr: number,
    size: number,
    currentEsp: number,
    thunkName: string,
    ...
    if (isStackRegion) {
      ...
      const overlapsReturnAddr =
        (addr >= currentEsp && addr < currentEsp + 4) ||
    ...
    if (
      this.callbackStubPoolBase !== 0 &&
      addr >= this.callbackStubPoolBase &&
      addr < this.callbackStubPoolEnd
    ) {
      Logger.error(
        LogCategory.THUNK,
  ...
  private _slowPathHandleDebugMarker(functionId: number, cpu: any): void {
    ...
    if (marker === 0x000a) {
      ...
      Logger.warn(
        LogCategory.SYSTEM,
        `[DIAG] DllMain call finished. Result (EAX): 0x${result.toString(16)}`,
      );
      return;
    }
    ...
    if (isException) {
      ...
      if (marker === 0x0004) {
        bootMark('first-game-code');
        self.postMessage({ type: 'gamebox_milestone', milestone: 'first_game_code' });
      }
  ...
  private _handleSehDispatchResult(cpu: any): void {
    ...
    if (!view || scratchAddr === 0) {
      Logger.error(LogCategory.SYSTEM, `SEH dispatch result: no scratch area or DataView`);
      return;
    }
    ...
    const lastHandlerResult =
      view.getUint32(scratchAddr + SEH_SCRATCH_LAYOUT.LAST_HANDLER_RESULT, true) >>> 0;
    const lastHandlerFrame =
      view.getUint32(scratchAddr + SEH_SCRATCH_LAYOUT.LAST_HANDLER_FRAME, true) >>> 0;
    const lastHandlerAddr =
      view.getUint32(scratchAddr + SEH_SCRATCH_LAYOUT.LAST_HANDLER_ADDR, true) >>> 0;
    const esp = cpu.reg32[4] >>> 0;
    ...
    if (result === 0) {
      ...
      if (this.unhandledExceptionFilterAddr !== 0 && this.cachedMem8) {
        ...
        if (curEsp >= 16) {
          ...
          view.setUint32(off, frameEsp, true);
          off += 4;
          mem[off++] = 0xb8; // MOV EAX, imm32
          view.setUint32(off, uefAddr, true);
          off += 4;
          mem[off++] = 0xff;
          mem[off++] = 0xe0; // JMP EAX
  ...
  private _handleRecoverablePageFault(cpu: any): void {
    ...
    if (this.sehDispatchStack.length > 0) {
      ...
      if (this.sehDispatchStack.length >= 2) {
        ...
        this._logSehCorruptionProtocol('seh_nested_fault_max_depth', faultingEip, faultAddr);
    ...
    if (view && faultGameEsp > 0 && faultGameEsp + 128 <= this.memLength) {
      for (let i = 0; i < 32; i++)
        faultStackDump.push(view.getUint32(faultGameEsp + i * 4, true) >>> 0);
    }
    ...
    if (view && faultingEip > 0 && faultingEip + 16 <= this.memLength) {
      ...
      for (let i = 0; i < 16; i++) {
        instrBytes.push(
          view
            .getUint8(faultingEip + i)
            .toString(16)
            .padStart(2, '0'),
        );
      }
      Logger.error(LogCategory.SYSTEM, `  EIP bytes: ${instrBytes.join(' ')}`);
    }
    ...
    if (view && gameEsp > 0 && gameEsp + 128 <= this.memLength) {
      ...
      for (let i = 0; i < 32; i++) {
        ...
        else if (val >= MEM_THUNK_CODE_BASE && val < MEM_THUNK_DATA_BASE + MEM_THUNK_DATA_SIZE)
          annotation = ' (thunk?)';
        else if (
          val >= 0x3e000000 &&
          val < 0x48000000 &&
        ...
        stackWords.push(
          `  [GameESP+0x${(i * 4).toString(16).padStart(2, '0')}] = 0x${val.toString(16).padStart(8, '0')}${annotation}`,
        );
      }
      Logger.error(
        LogCategory.SYSTEM,
    ...
    // Dump recent thunk calls (ring buffer) for crash diagnosis
    {
      ...
      Logger.error(
        LogCategory.SYSTEM,
        `#PF crash trace — last ${lines.length} thunks:\n${lines.join('\n')}`,
      );
    }
  ...
  private _evaluateSimpleFilter(
    ...
    if (
      filterAddr + 19 <= this.memLength &&
      mem[filterAddr] === 0x8b &&
      mem[filterAddr + 1] === 0x45 &&
      mem[filterAddr + 2] === 0xec && // MOV EAX,[EBP-14h]
      mem[filterAddr + 3] === 0x8b &&
      mem[filterAddr + 4] === 0x00 && // MOV EAX,[EAX]
      mem[filterAddr + 5] === 0x8b &&
      mem[filterAddr + 6] === 0x00 && // MOV EAX,[EAX]
      mem[filterAddr + 7] === 0x3d && // CMP EAX, imm32
      mem[filterAddr + 12] === 0x0f &&
      mem[filterAddr + 13] === 0x94 &&
      mem[filterAddr + 14] === 0xc0 && // SETE AL
      mem[filterAddr + 15] === 0x0f &&
      mem[filterAddr + 16] === 0xb6 &&
      mem[filterAddr + 17] === 0xc0 && // MOVZX EAX, AL
      ...
      const cmpImm =
        (mem[filterAddr + 8] |
          (mem[filterAddr + 9] << 8) |
          (mem[filterAddr + 10] << 16) |
          (mem[filterAddr + 11] << 24)) >>>
        0;
    ...
    if (
      filterAddr + 17 <= this.memLength &&
      mem[filterAddr] === 0x8b &&
      mem[filterAddr + 1] === 0x45 &&
      mem[filterAddr + 2] === 0xec &&
      mem[filterAddr + 3] === 0x8b &&
      mem[filterAddr + 4] === 0x08 &&
      mem[filterAddr + 5] === 0x8b &&
      mem[filterAddr + 6] === 0x09
    ) {
  ...
  private _handleRecoverableDivisionError(cpu: any): void {
    ...
    if (esp + 12 > this.memLength) {
      Logger.error(
        LogCategory.SYSTEM,
        `#DE: Cannot recover - ESP=0x${esp.toString(16)} out of range`,
      );
      return;
    }
    ...
    if (opnd && cpu.reg32) {
      ...
      if (dumpFrom + 0x40 <= this.memLength) {
        ...
        for (let i = 0; i < 0x40; i += 4) {
          words.push(
            `+${i.toString(16).padStart(2, '0')}=0x${(view.getUint32(dumpFrom + i, true) >>> 0).toString(16).padStart(8, '0')}`,
          );
        }
        divDiag =
    ...
    try {
      ...
      if (opnd && cpu.reg32) {
        const base = cpu.reg32[opnd.regIdx] >>> 0;
        const nextPtr =
          this.cachedDataView && base + 4 <= this.memLength ? view.getUint32(base, true) >>> 0 : 0;
        ...
        if (h && h.length) {
          histDiag =
            ...
            h
              ...
              .map(
                (e: any) =>
                  `\n    ${e.t} ${e.op.padEnd(5)} ${e.addr} sz=${e.size}${e.overlaps ? ' *' : ''}  ${e.bt}`,
              )
              .join('');
        } else {
   ...
   */
  private _decodeDivMemOperand(
    mem: Uint8Array,
    eip: number,
  ): { regIdx: number; name: string; disp: number } | null {
  ...
  private _slowPathHandleInvalidId(functionId: number, cpu: any): void {
    ...
    if (mem8 && esp > 0 && esp < mem8.length - 16) {
      ...
      if (this.isDataViewValid()) {
        ...
        Logger.error(
          LogCategory.THUNK,
          `Stack at ID ${functionId} error: ${stackValues.join(' ')}`,
        );
      }
    }
    ...
    if (functionId !== 0) {
      ...
      if (!stub) {
        ...
        Logger.error(
          LogCategory.THUNK,
          `?? Thunk ID ${functionId} (0x${functionId.toString(16)}) not found! EIP=0x${eip.toString(16)} ESP=0x${esp.toString(16)}`,
        );
        Logger.error(
          LogCategory.THUNK,
          `  > Registers: EAX=0x${eax.toString(16)} EBX=0x${ebx.toString(16)} ECX=0x${ecx.toString(16)} EDX=0x${edx.toString(16)} ESI=0x${esi.toString(16)} EDI=0x${edi.toString(16)} EBP=0x${ebp.toString(16)}`,
        );
        ...
        if (mem8 && eip > 0 && eip < mem8.length - 16) {
          ...
          Logger.error(
            LogCategory.THUNK,
            `  > Bytes at EIP 0x${eip.toString(16)}: ${codeSlice.map((b: number) => b.toString(16).padStart(2, '0')).join(' ')}`,
          );
        }

        // Check if ESP change is massive
        const lastThunkEsp =
          this.activeAsyncThunks.size > 0
            ? Array.from(this.activeAsyncThunks.values())[0].esp
            : undefined;
        if (lastThunkEsp !== undefined) {
          ...
          if (diff > 0x100) {
            Logger.error(
              LogCategory.THUNK,
              `  > MASSIVE Stack Change Detected! Prev ESP=0x${lastThunkEsp.toString(16)}, Current ESP=0x${esp.toString(16)} (Diff: +0x${diff.toString(16)})`,
            );
          }
        }
        ...
        if (closeStubs.length > 0) {
          ...
          for (const stub of closeStubs.slice(0, 5)) {
            Logger.error(
              LogCategory.THUNK,
              `    - ID=${stub.functionId} (0x${stub.functionId.toString(16)}) > ${stub.dllName}:${stub.functionName} (diff=${Math.abs(stub.functionId - functionId)})`,
            );
          }
        }
      }
  ...
  private _tryDllForward(functionId: number): ThunkImplementation | null {
    ...
    if (pending) {
      ...
      const targetStub = allStubs.find(
        (s) => s.dllName.toLowerCase() === targetDll && s.functionName === targetFuncName,
      );
  ...
  private applyPendingFastPathForStub(functionId: number, stub: ThunkStub): void {
    ...
    const pending =
      ...
      (normalizedKey !== exactKey
        ? this.pendingFastPathRegistrations.get(normalizedKey)
        : undefined);
  ...
  private applyPendingWriteBufferForStub(stub: ThunkStub): void {
    ...
    const pending =
      ...
      (normalizedKey !== exactKey
        ? this.pendingWriteBufRegistrations.get(normalizedKey)
        : undefined);
    ...
    if (pending.structCapture) {
      this.registerStructCaptureWriteBufferFunction(
        ...
        pending.functionName,
        pending.argCount,
        pending.structCapture.ptrArgIndex,
        pending.structCapture.payloadDwords,
        pending.handler,
      );
      return;
    }
  ...
  private _tryPendingRegistration(functionId: number): ThunkImplementation | null {
    ...
    if (!pending && !/[aw]$/i.test(stub.functionName)) {
      pending = this.pendingRegistrations.get(
        `${stub.dllName}:${stub.functionName}A`.toLowerCase(),
      );
    }
    if (!pending && /a$/i.test(stub.functionName)) {
      pending = this.pendingRegistrations.get(
        `${stub.dllName}:${stub.functionName.slice(0, -1)}`.toLowerCase(),
      );
    }
    if (pending) {
  ...
  private _slowPathMissingImplementation(functionId: number, cpu: any, name: string): void {
    ...
    const caller =
      this.cachedDataView && this.isDataViewValid() && esp < this.memLength - 4
        ? this.cachedDataView.getUint32(esp, true) >>> 0
        : 0;
    ...
    if (stub) {
      ...
      stubRegistry.record(
        ci > 0 ? name.slice(0, ci) : '',
        ci > 0 ? name.slice(ci + 1) : name,
        functionId,
        caller,
      );
    }
  ...
  private _slowPathInvalidReturn(
    id: number,
    name: string,
    esp: number,
    val: number,
   ...
   */
  private _reportInvalidReturnCrash(
    name: string,
    esp: number,
    retAddr: number,
    cpu: any,
  ...
  applyPendingRegistrations(): void {
    ...
    for (const [, pending] of this.pendingRegistrations.entries()) {
      ...
      for (const stub of matchingStubs) {
        ...
        if (id < MAX_THUNK_ID) {
          ...
          Logger.verbose(
            LogCategory.THUNK,
            `Applied pending registration [${id}] ${pending.dllName}:${pending.functionName}`,
          );
          applied++;
        }
      }
    ...
    for (const [, pending] of this.pendingFastPathRegistrations.entries()) {
      for (const stub of this.findStubsByName(pending.dllName, pending.functionName)) {
        if (stub.functionId < MAX_THUNK_ID) {
          ...
          Logger.verbose(
            LogCategory.THUNK,
            `Applied pending fast-path [${stub.functionId}] ${pending.dllName}:${pending.functionName}${pending.trivial ? ' (trivial)' : ''}`,
          );
          applied++;
        }
      }
    ...
    for (const [, pending] of this.pendingConstStubRegistrations.entries()) {
      if (this.findStubsByName(pending.dllName, pending.functionName)[0]) {
        this.registerConstantReturnStub(
          pending.dllName,
          pending.functionName,
          pending.value,
          pending.popBytes,
