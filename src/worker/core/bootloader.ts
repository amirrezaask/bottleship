...
export function createBootloader(
  ...
  // Report the exact boundary before the first game-owned module entrypoint.
  // This is a one-time startup marker; it runs before the first DllMain when
  // imports exist, otherwise immediately before the EXE entrypoint.
  trampoline.push(
    0xb8,
    0x04,
    0x00,
    0xad,
    0xde, // MOV EAX, 0xDEAD0004
    0xba,
    0x77,
    0xb0,
    0x00,
  ...
  // Write handlers
  finalBuffer.set(createHaltHandlerBytes(0xdead00ee, true), hGenericOff);
  ...
  finalBuffer.set(createHaltHandlerBytes(0xdead02ee, false), hInt2eOff);
  finalBuffer.set(createHaltHandlerBytes(0xdead0080, false), hInt80Off);
  finalBuffer.set(createRecoverableHandlerBytes(0xdead0000), hDeOff); // #DE - recoverable via IRET
...
function createHaltHandlerBytes(thunkId: number, dumpEip: boolean = false): Uint8Array {
  // Basic handler:
  // MOV EAX, thunkId
  // MOV EDX, 0xB077
  // OUT DX, EAX
  ...
  if (dumpEip) {
    ...
    const hasErrorCode = thunkId === 0xdead000d || thunkId === 0xdead000e;
...
function createRecoverablePfHandler(): Uint8Array {
  return new Uint8Array([
    ...
    0x83,
    0xc4,
    0x04, // ADD ESP, 4   (pop error code — IRET won't pop it)
...
function setIDTEntry(view: DataView, index: number, handlerLinear: number) {
  const off = index * 8;
  ...
  view.setUint16(off + 6, (handlerLinear >>> 16) & 0xffff, true); // offset high
}
